...
### 0-1. 開発環境セットアップ ✅
- [x] リポジトリにpocディレクトリを作成
- [x] Node.js + TypeScript環境の初期化
- [ ] 必要なAPIキーの取得
  - [ ] OpenAI API（Whisper用）
...
- [x] ブラウザでマイク録音するHTMLページ作成
- [x] MediaRecorder APIで音声をBlob化
- [x] Whisper APIへ音声を送信（APIエンドポイント実装済み）
- [x] 認識結果をテキスト表示
- [ ] 認識精度の確認（複数パターンでテスト）← **APIキー設定後に検証**
- [x] ElevenLabs/ブラウザTTSでテキストを音声化
- [x] 音声ファイルをブラウザで再生
- [ ] **検証結果を記録**

### 0-3. PoC-2: LLM会話の成立 🚧
- [x] Claude APIへの接続（APIエンドポイント実装済み）
- [x] 「孫っち」ペルソナのシステムプロンプト作成
- [x] 基本的な会話のテスト用UI作成
- [x] ストリーミングレスポンスの実装
- [ ] 日本語での自然な応答確認 ← **APIキー設定後に検証**
- [ ] **検証結果を記録**
...
- [x] 音声入力→テキスト化→LLM→音声出力の一連のフロー実装
- [ ] 応答時間の計測 ← **APIキー設定後に検証**
- [ ] ボトルネックの特定
- [ ] **Go/No-Go判断の実施**
